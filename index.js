...
async function generateDigest(feedData) {
  ...
  const LINK_RULES = `
链接格式规则（非常重要，必须严格遵守）：
- 每条内容的链接必须直接附在人名或标题上，格式：[人名 · 机构](url)：内容描述
- 播客条目格式：[播客名 · 嘉宾名](url)：内容描述
- 严禁把链接单独放在一行（不能出现单独一行只有一个链接文字的情况）
- 严禁在链接后面加 (机构名) 括号
- 严禁出现裸 URL
- 不要在链接外面套粗体，直接写 [名字](url) 即可
- 正确示例：[Andrej Karpathy · ex-OpenAI](url)：他认为...
- 错误示例：Andrej Karpathy 认为...
[查看原文](url)`;
...
2. 「播客精华 🎙️」：如有播客，先用一行写 [播客名 · 集标题](播客url) 作为标题，然后列出核心观点（普通文字，不需要链接）；无则写「本周暂无播客更新」
...
内容结构（按此顺序）：
1. 「今日必看 ⭐」：最重要的 3 条，每条格式：[人名 · 机构](url)：一句话说明为什么值得看
2. 「播客精华 🎙️」：如有播客，先用一行写 [播客名 · 集标题](播客url) 作为标题，然后列出 3-5 个核心观点（普通文字，不需要链接）；无则写「今日暂无播客更新」
